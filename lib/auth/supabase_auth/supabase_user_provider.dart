...
class SproutifyFarmAdminSupabaseUser extends BaseAuthUser {
  SproutifyFarmAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sproutifyFarmAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SproutifyFarmAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
